...
(function ($) {
    ...
    const ChatHistoryManager = {
        ...
        bindEvents: function () {
            ...
            // Navigate from search result (switch to conversation when user clicks "View Conversation" in search)
            $(document).on('aiBotKitSearch:navigateToResult', function (e) {
                const detail = (e.originalEvent && e.originalEvent.detail) ? e.originalEvent.detail : (e.detail || {});
                const conversationId = detail.conversationId;
                if (conversationId && self.switchConversation) {
                    self.switchConversation(conversationId);
                    self.closeHistoryPanel();
                }
            });
            ...
            // New conversation button (in history panel footer)
            $(document).on('click', '.ai-botkit-new-conversation-btn', function (e) {
                e.preventDefault();
                if (typeof window.ai_botkit !== 'undefined' && typeof window.ai_botkit.startNewConversation === 'function') {
                    window.ai_botkit.startNewConversation();
                }
                self.closeHistoryPanel();
            });
            ...
            const $item = $(`
                <div class="ai-botkit-conversation-item ${isActive ? 'is-active' : ''} ${favoriteClass}"
                    ...
                    <div class="ai-botkit-conversation-actions">
                        <button type="button" class="ai-botkit-export-pdf-btn"
                                data-conversation-id="${conversation.id}"
                                title="${this.config.i18n.exportPdf || 'Export PDF'}"
                                aria-label="${this.config.i18n.exportPdf || 'Export PDF'}">
        ...
        loadConversationMessages: function (conversationData) {
            ...
            // Update the current conversation ID in the main chat so new messages save to this conversation
            if (typeof window.ai_botkit !== 'undefined' && window.ai_botkit.setConversationId) {
                window.ai_botkit.setConversationId(conversationData.conversation_id);
            }
